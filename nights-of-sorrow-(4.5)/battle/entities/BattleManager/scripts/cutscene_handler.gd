...
const ACTION_EFFECTS = preload("uid://ufew18dl63uv")
...
func call_script(script: Callable, args: Array):
	script.callv(args)
	action_end()
...
func cast_action_func(action: Action, user: BattleUnit, targets: Array, area: ActionArea, damage = null):
	...
	if (damage == null):
		damage = BattleHandler.calc_action_default_damage(action, user)
